    ...
    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-secondary overflow-hidden shadow-2xl rounded-3xl p-10 text-white mb-10 relative group border border-primary/20">
            <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div>
                    <h1 class="text-4xl font-extrabold mb-2 tracking-tight text-primary">{{ __('messages.edit_component') ?? __('Edit Task') }}</h1>
                    <p class="text-gray-300 text-lg opacity-90">{{ __('Update task details and manage assignment.') }}</p>
                </div>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-gray-100/50 overflow-hidden transition-all duration-500">
            <form method="POST" action="{{ route('client.tasks.update', $task) }}" class="p-12 space-y-10">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <!-- Status -->
                    <div class="md:col-span-2 space-y-3">
                        <label for="status" class="block text-xs font-black text-gray-400 uppercase tracking-[0.2em]">{{ __('messages.status') ?? __('Status') }}</label>
                        <div class="grid grid-cols-3 gap-4">
                            @foreach(['todo', 'in_progress', 'done'] as $status)
                                <label class="cursor-pointer group">
                                    <input type="radio" name="status" value="{{ $status }}" {{ old('status', $task->status) == $status ? 'checked' : '' }} class="peer hidden">
                                    <div class="p-4 rounded-2xl border-2 border-gray-100 bg-gray-50 text-center transition-all duration-300 peer-checked:border-primary peer-checked:bg-primary/10 group-hover:border-gray-200">
                                        <span class="block text-[10px] font-black uppercase tracking-widest text-gray-400 peer-checked:text-secondary">{{ __($status) }}</span>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                ...
                <div class="pt-10 border-t border-gray-50 flex items-center justify-center gap-6">
                    ...
                    <button type="submit" 
                            class="inline-flex items-center px-16 py-5 bg-primary hover:bg-primary/90 text-secondary text-base font-black rounded-3xl shadow-[0_20px_40px_rgba(var(--color-primary-rgb),0.2)] transition-all duration-300 hover:scale-105 active:scale-95 group/submit">
                        <svg class="w-6 h-6 me-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                        {{ __('messages.update') }}
                    </button>
                </div>
            </form>

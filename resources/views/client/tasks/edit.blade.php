...
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">{{ __('Edit Task') }}</h1>
        <p class="mt-2 text-sm text-gray-500">{{ __('Update task details and manage assignment.') }}</p>
    </div>
    ...
    <form action="{{ route('client.tasks.update', $task) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label for="status" class="block text-sm font-bold text-gray-700">{{ __('Status') }}</label>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm py-2.5">{{ old('description', $task->description) }}</textarea>
        </div>

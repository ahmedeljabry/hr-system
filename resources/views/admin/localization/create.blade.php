...
<div class="pt-8 pb-12" x-data="{ 
    jobs: [{ occupation_code: '', job_title_ar: '', job_title_en: '' }],
    addJob() {
    ...
    <div class="max-w-4xl mx-auto">
        <x-dashboard-sub-header 
            :title="__('Add Localization Decision')" 
            :subtitle="__('Create a new localization requirement for workforce.')"
        />

        <form action="{{ route('admin.localization.store') }}" method="POST" enctype="multipart/form-data" class="mt-8 space-y-8">
            @csrf
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" step="0.01" name="saudi_percentage" required class="w-full px-4 py-3 rounded-xl border-gray-200 focus:ring-secondary focus:border-secondary transition-all" placeholder="e.g. 25.00">
                        ...
                        <div class="p-6 rounded-2xl bg-gray-50 border border-gray-100 relative group">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input type="text" :name="`jobs[${index}][occupation_code]`" x-model="job.occupation_code" required class="w-full px-4 py-2.5 rounded-xl border-gray-200 focus:ring-secondary focus:border-secondary transition-all" placeholder="Code">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1.5">{{ __('Job Title (AR)') }}</label>
                                    <input type="text" :name="`jobs[${index}][job_title_ar]`" x-model="job.job_title_ar" required class="w-full px-4 py-2.5 rounded-xl border-gray-200 focus:ring-secondary focus:border-secondary transition-all text-right" dir="rtl" placeholder="العنوان بالعربي">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1.5">{{ __('Job Title (EN)') }}</label>
                                    <input type="text" :name="`jobs[${index}][job_title_en]`" x-model="job.job_title_en" class="w-full px-4 py-2.5 rounded-xl border-gray-200 focus:ring-secondary focus:border-secondary transition-all" placeholder="Title (Optional)">
                                </div>
                            </div>
                        </div>
                    </template>
                    ...
                    <div class="border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center transition-colors hover:border-secondary/50 group bg-gray-50/30">
                        ...
                        <div x-show="files.length === 0">
                            <label for="files" class="cursor-pointer">
                                ...
                                <p class="text-gray-500 font-bold">{{ __('Click to upload images or documents') }}</p>
                                <p class="text-[10px] text-gray-400 uppercase tracking-widest mt-2">{{ __('PDF, JPG, PNG, DOCX (Max 10MB)') }}</p>
                            </label>
                        </div>
                        ...
                        <div x-show="files.length > 0" class="text-start space-y-4 animate-in fade-in zoom-in duration-300">
                            <div class="flex items-center justify-between border-b border-gray-100 pb-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-secondary font-black text-sm uppercase tracking-widest" x-text="`${files.length} {{ __('files selected') }}`"></span>
                                </div>

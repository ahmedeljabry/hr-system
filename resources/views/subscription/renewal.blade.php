...
<div class="flex flex-col items-center justify-center py-16">
    <div class="w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-red-50 overflow-hidden transform transition-all hover:scale-[1.01]">
        <div class="bg-red-500 p-8 text-center text-white relative">
            ...
            <h1 class="text-3xl font-extrabold mb-2">تنبيه: الاشتراك غير نشط</h1>
            <p class="text-red-100 opacity-90 italic">Subscription Access Restricted</p>
        </div>
        
        <div class="p-10 text-center">
            <p class="text-gray-700 text-lg leading-relaxed mb-6 font-bold">
                عذرًا، يرجى ملاحظة أن اشتراك شركتك <span class="text-red-600">({{ Auth::user()->client?->name }})</span> غير نشط في الوقت الحالي.
            </p>
            
            <div class="bg-gray-50 border border-gray-100 rounded-2xl p-6 mb-8 text-start">
                <h3 class="text-gray-400 uppercase text-xs font-bold tracking-widest mb-4">تفاصيل الاشتراك / Details</h3>
                <div class="flex justify-between items-center mb-3">
                    <span class="text-sm font-semibold text-gray-500">الحالة / Status</span>
                ...
                <div class="flex justify-between items-center">
                    <span class="text-sm font-semibold text-gray-500">تاريخ الانتهاء / End Date</span>
            ...
            <p class="text-gray-500 text-sm mb-10 leading-loose">
                يرجى التواصل مع إدارة النظام عبر البريد الإلكتروني أو الهاتف لتسوية الرصيد وتفعيل الخدمة. بمجرد تفعيل الاشتراك، ستتمكن من الوصول لجميع الميزات تلقائيًا.
            </p>
            
            <div class="flex gap-4">
                <a href="mailto:support@hr-system.com" class="flex-1 bg-gray-900 text-white font-bold py-4 rounded-2xl hover:bg-black transition text-sm">تواصل معنا</a>
                <form action="{{ route('logout') }}" method="POST" class="flex-1">
                    @csrf
                    <button type="submit" class="w-full bg-white text-gray-700 border border-gray-200 font-bold py-4 rounded-2xl hover:bg-gray-50 transition text-sm">تسجيل الخروج</button>
                </form>
            </div>
        </div>
    </div>

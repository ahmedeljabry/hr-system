...
<div class="flex h-screen w-full">
    ...
    <div class="w-full md:w-1/2 flex flex-col justify-center items-center p-8 bg-surface overflow-y-auto">
        <div class="w-full max-w-md">
            <div class="mb-8 text-start">
                ...
                <h1 class="text-3xl font-outfit font-bold text-text-main mb-2">{{ $title }}</h1>
                @if($subtitle)
    ...
    <!-- Imagery Panel -->
    <div class="hidden md:flex md:w-1/2 bg-primary relative overflow-hidden items-center justify-center">
        ...
        <div class="relative z-10 px-12 text-center text-white">
            @if(isset($branding))
                {{ $branding }}
            @else
                <h2 class="text-4xl font-outfit font-bold mb-4">{{ __('auth.welcome_back') ?? 'Welcome to the platform' }}</h2>

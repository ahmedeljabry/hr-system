...
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">{{ __('Add Asset') }}</h1>
        <p class="mt-2 text-sm text-gray-500">{{ __('Register a new organizational asset and assign it to an employee.') }}</p>
    </div>
    ...
    <form action="{{ route('client.assets.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 space-y-6">
        @csrf
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="type" class="block text-sm font-bold text-gray-700">{{ __('Property Type') }}</label>
                <input type="text" name="type" id="type" value="{{ old('type') }}" required placeholder="e.g. Laptop, Phone"
            ...
            <div>
                ...
                <input type="text" name="serial_number" id="serial_number" value="{{ old('serial_number') }}" placeholder="Unique ID"
                    class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm py-2.5">
            </div>
        </div>

        <div>
            <label for="employee_id" class="block text-sm font-bold text-gray-700">{{ __('Custodian (Employee)') }}</label>
